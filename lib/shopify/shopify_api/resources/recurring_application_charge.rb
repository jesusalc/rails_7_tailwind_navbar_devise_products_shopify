...
module ShopifyAPI
  class RecurringApplicationCharge < Base
    ...
    class << self
      ...
      %i[pending cancelled accepted declined].each do |status|
    ...
    def cancel
      load_attributes_from_response(destroy)
    end
    ...
    def customize(customize_recurring_app_charge_params = {})
      load_attributes_from_response(put(:customize,
                                        recurring_application_charge: customize_recurring_app_charge_params))
    end
  end
end
